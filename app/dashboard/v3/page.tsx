...
import { BatchDashboardContent } from '@/components/dashboard/batch-dashboard-content'
...
export default async function BatchDashboardPage() {
  ...
  console.log('🚀 [BATCH] Cargando dashboard con sistema batch para:', session.user.name)
  console.log('🏫 [BATCH] Aula principal:', session.user.aulaInfo?.primaryAula)
  ...
  try {
    // Crear cliente basado en sesión (para obtener cursos del profesor)
    ...
    if (isConnected) {
      ...
      try {
        console.log('🎯 [BATCH] Obteniendo cursos-grupos del profesor...')
        const teacherCourseGroups = await sessionClient.getTeacherCourseGroups()
        
        if (!teacherCourseGroups || teacherCourseGroups.length === 0) {
          console.warn('⚠️ [BATCH] No se obtuvieron cursos-grupos, intentando método legacy...')
